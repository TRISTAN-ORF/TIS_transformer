...
class TranscriptSeqRiboEmbCustom(pl.LightningModule):
    ...
    def forward(self, batch, y_mask):
        ...
        x = self.transformer(x, attention_mask=x_mask)
...
dim_head = 16
local_attn_heads = 8
local_window_size = 203

tis_tr = TranscriptSeqRiboEmbCustom(True, False, 8, 0.001, 0.96, 1500, max_seq_len, dim, depth, 
                                    heads, dim_head, False, 80, 1000, True,
...
tr_loader = h5pyDataModule(h5py_path, exp_path, ribo_path, y_path, x_seq, ribo_offset, 
                           x_id_path, contig_path, val=val, test=test,  max_transcripts_per_batch=400, max_seq_len=max_seq_len,
                           max_memory=max_memory, num_workers=4, cond_fs=None, leaky_frac=0.05, collate_fn=collate_fn)

checkpoint_callback = ModelCheckpoint(monitor="val_loss", 
                                        filename="{epoch:02d}_{val_loss:.2f}", save_top_k=1, mode="min")
tb_logger = pl.loggers.TensorBoardLogger('.', os.path.join('DNA_former_logs', 'benchmark_200_new'))
